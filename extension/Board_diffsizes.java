 ...
 * Project 5
 * 
 * Description:  The Board class holds the array of Cells that make up the Sudoku board. 
                ...
                *  It contains a constructor to initialize the Board with a value, and various getter and setter methods
...
public class Board_diffsizes {
    ...
    public Board_diffsizes(int size) {
        this.size = size;
        finished = false;
    ...
    // Constructor with default size of 9
    public Board_diffsizes() {
        this(9);
    }

    // Constructor with specified size and initial number of locked cells
    public Board_diffsizes(int initial_numLocked, int size) {
        this(size);
        ...
        for (int lockedCount = 0; lockedCount < initial_numLocked; ) {
            ...
            if (!board[row][col].isLocked()) {
                int value = rand.nextInt(size) + 1; // Generate a rand value in the range [1, size]
...
public boolean validValue(int row, int col, int value){
    // if the value is not within the range [1, size]
    ...
    for (int col_index = 0; col_index < size; col_index++) {
        if (col_index != col && board[row][col_index].getValue() == value) {
            return false;
        }
    }
    ...
    for (int row_index = 0; row_index < size; row_index++) {
        if (row_index != row && board[row_index][col].getValue() == value) {
            return false;
        }
    }

    // Calculate the dimensions of the inner square
    int innerSize = (int) Math.sqrt(size);

    // Check if the value is unique in its local inner square
    int startRow = row - row % innerSize;
    int startCol = col - col % innerSize;
    ...
    public String toString(){
        ...
        int innerSize = (int) Math.sqrt(size); // Calculate the size of the inner square
        
        for (int i = 0; i < size; i++) {
            // Add horizontal separators for every innerSize-th row
            if (i % innerSize == 0 && i != 0) {
                for (int k = 0; k < size + innerSize - 1; k++) {
            ...
            for (int j = 0; j < size; j++) {
                // Add vertical separators for every innerSize-th column
                if (j % innerSize == 0 && j != 0) {
                    board_representation += "| ";
                }
